...
$(document).ready(function(){
    ...
    {% if native_videos_enabled %}
        // Fancybox v3 destroys and recreates iframe DOM when opening/closing modals,
        // which causes videos to restart from the beginning. To support resume playback,
        // we save the playback position (videoSavedTimes), destroy the old iframe on
        // navigation, and recreate it with a startTime parameter when the user returns.
        // This complexity would not be needed with Fancybox v5 (used by other themes),
        // which preserves DOM content without reloading iframes.
        ...
        $('.js-product-video-modal .js-external-video-iframe-container').each(function() {
            var container = $(this);
            var iframe = container.find('.js-external-video-iframe');
            if (iframe.length) {
                container.attr('data-video-src', iframe.data('src'));
                iframe.parent().remove();
            }
        });

        function initAllVideos() {
            $(".js-external-video-iframe").each(function() {
                try {
        ...
        function pauseAllVideos() {
            ...
            Object.keys(modalPlayers).forEach(function(uid) {
                try { modalPlayers[uid].pause(); } catch(e) {}
            });
        }

        function cleanupModalPlayer(uid) {
            if (modalTimeouts[uid]) {
                clearTimeout(modalTimeouts[uid]);
                delete modalTimeouts[uid];
            }
            if (modalIntervals[uid]) {
                clearInterval(modalIntervals[uid]);
                delete modalIntervals[uid];
            }
            if (modalPlayers[uid]) {
                try { modalPlayers[uid].destroy(); } catch(e) {}
                delete modalPlayers[uid];
            }
        }

        function createModalIframe(iframeContainer, uid, src, allowAutoplay) {
            var iframeId = 'video-modal-' + uid;
            var aspectPadding = iframeContainer.data('aspect-ratio') || '56.25%';
            var aspectDiv = $('<div>').addClass('dynamic-video-wrapper').css({position: 'relative', 'padding-top': aspectPadding});
            var allowPolicy = 'accelerometer; gyroscope; encrypted-media; picture-in-picture;';
            if (allowAutoplay) {
                allowPolicy = 'accelerometer; gyroscope; autoplay; encrypted-media; picture-in-picture;';
            }
            ...
            }).css({border: 'none', position: 'absolute', top: 0, height: '100%', width: '100%'});
            aspectDiv.append(newIframe);
            iframeContainer.append(aspectDiv);
            return iframeId;
        }

        function resetModalVideos() {
            $('.js-product-video-modal .js-play-native-button').each(function() {
                var btn = $(this);
                var wrapper = btn.closest('.product-native-video-container');
                if (!wrapper.length) return;
                var iframeContainer = wrapper.find('.js-external-video-iframe-container');
                var uid = iframeContainer.data('video_uid');
                var image = wrapper.find('img.video-image');
                var videoSrc = iframeContainer.attr('data-video-src');

                if (uid) cleanupModalPlayer(uid);
                iframeContainer.find('iframe').remove();
                iframeContainer.find('.dynamic-video-wrapper').remove();

                if (videoSrc && uid && videoSavedTimes[uid] && videoSavedTimes[uid] > 0) {
                    var savedTime = Math.floor(videoSavedTimes[uid]);
                    var separator = videoSrc.indexOf('?') > -1 ? '&' : '?';
                    var resumeSrc = videoSrc.replace('autoplay=true', 'autoplay=false') + separator + 'startTime=' + savedTime + 's';

                    createModalIframe(iframeContainer, uid, resumeSrc, false);
                    iframeContainer.show();
                    image.hide();
                    btn.hide().removeClass('d-md-block');
                } else {
                    iframeContainer.hide();
                    image.show();
                    btn.css('display', '').addClass('d-md-block');
                }
            });
        }

        $(".js-play-native-button").on("click", function() {
            pauseAllVideos();
            var link = $(this);
            var wrapper = link.closest('.product-native-video-container');
            var iframeContainer = wrapper.find('.js-external-video-iframe-container');
            var image = wrapper.find('img.video-image');
            var uid = iframeContainer.data('video_uid');
            var isModal = wrapper.closest('.js-product-video-modal').length > 0;

            if (isModal) {
                var src = iframeContainer.attr('data-video-src');
                if (!src) return;

                if (uid && videoSavedTimes[uid] && videoSavedTimes[uid] > 0) {
                    var savedTime = Math.floor(videoSavedTimes[uid]);
                    var separator = src.indexOf('?') > -1 ? '&' : '?';
                    src = src + separator + 'startTime=' + savedTime + 's';
                }
                ...
                var iframeId = createModalIframe(iframeContainer, uid, src, true);
                iframeContainer.show();
                image.hide();
                ...
                if (uid) {
                    var trackUid = uid;
                    var trackIframeId = iframeId;
                    // Cloudflare Stream SDK needs the iframe to be fully loaded before
                    // initializing the player. setTimeout gives the iframe time to render
                    // and load the video source before calling Stream().
                    modalTimeouts[trackUid] = setTimeout(function() {
                        delete modalTimeouts[trackUid];
                        var el = document.getElementById(trackIframeId);
                        if (!el) return;
                        try {
                            var player = Stream(el);
                            modalPlayers[trackUid] = player;

                            modalIntervals[trackUid] = setInterval(function() {
                                try {
                                    var t = player.currentTime;
                                    if (typeof t === 'number' && t > 0) {
                                        videoSavedTimes[trackUid] = t;
                                    }
                                } catch(e) {
                                    cleanupModalPlayer(trackUid);
                                }
                            }, 1000);
                        } catch(e) {}
                    }, 3000);
                ...
                var iframe = iframeContainer.find('.js-external-video-iframe');
    ...
    $(document).on("click", ".js-addtocart:not(.js-addtocart-placeholder)", function (e) {
        ...
        if (isCrossSelling) {
            ...
            var price = $productContainer.find('.js-price-display').first().text();
        } else {
            var imageSrc = $(this).closest('.js-quickshop-container').find('img').attr('srcset');
            var name = $productContainer.find('.js-item-name').text();
            var price = $productContainer.find('.js-price-display').first().text().trim(); 
        }
