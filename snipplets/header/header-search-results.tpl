<ul class="search-suggest-list mt-3 p-1">
        ...
        <li class="search-suggest-item container-fluid">
            <a href="{{ product.url }}" class="search-suggest-link row justify-content-md-center">
                <div class="search-suggest-image-container col-xs-auto">
                    {{ product.featured_image | product_image_url("thumb") | img_tag(product.featured_image.alt, {class: 'search-suggest-image'}) }}
                </div>
                <div class="search-suggest-text col">
                    ...
                    {% if product.display_price %}
                        ...
                        {% else %}
                            <p {% if product.isSubscribable() %}class="mb-0"{% endif %}>
                                {{ product.price | money }}
    ...
    {% endfor %}
    <a href="#" class="js-search-suggest-all-link btn btn-primary d-block">{{ 'Ver más resultados' | translate }}</a>
</ul>
