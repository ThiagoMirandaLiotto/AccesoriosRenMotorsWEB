    ...
    <div class="item-product-card">
        ...
        {% endif %}
            ...
            {% set floating_elements %}
                {% if not (full_item or reduced_item) %}
                    ...
                    {% if settings.quick_shop and product.available and product.display_price %}
                        ...
                        {% set quickshop_button_classes = 'btn btn-primary item-btn-quickshop' %}
                        ...
                        <div class="item-actions">
                            ...
                            {% if product.isSubscribable() %}
                                ...
                                <div class="item-submit-container btn btn-primary">
                                    <a href="{{ product_url_with_selected_variant }}" class="{{ quickshop_button_classes }}" title="{{ quickshop_title }}" aria-label="{{ quickshop_title }}">
                            ...
                            {% else %}
                                {% if product.variations %}
                                    ...
                                    <div class="item-submit-container btn btn-primary">
                                        <a data-toggle="#quickshop-modal" data-modal-url="modal-fullscreen-quickshop" class="js-quickshop-modal-open {% if slide_item %}js-quickshop-slide{% endif %} js-modal-open js-fullscreen-modal-open {{ quickshop_button_classes }}" title="{{ 'Compra rápida de' | translate }} {{ product.name }}" aria-label="{{ 'Compra rápida de' | translate }} {{ product.name }}" >{% include "snipplets/svg/shopping-bag.tpl" with {svg_custom_class: 'js-quickshop-bag'} %}</a>
                                    </div>
                                {% else %}
                                    ...
                                    <form class="js-product-form" method="post" action="{{ store.cart_url }}">
                                        ...
                                        <div class="js-quickshop-add-container item-submit-container btn btn-primary">
                                            <input type="submit" class="js-addtocart js-prod-submit-form js-quickshop-icon-add {{ quickshop_button_classes }} {{ state }}" value="" aria-label="{{ texts[state] | translate }}" {% if state == 'nostock' %}disabled{% endif %} />
            ...
            {% if 
                ...
                and not (full_item or reduced_item)) 
            ...
            <div class="{% if full_item %}col-8 col-md-9 px-3 {% endif %}item-description {% if reduced_item %}p-2 m-1{% endif %}" data-store="product-item-info-{{ product.id }}">
                ...
                <a href="{{ product_url_with_selected_variant }}" title="{{ product.name }}" class="item-link">
                    ...
                    {% if product.display_price %}
                        <div class="item-price-container mb-{% if reduced_item %}0{% else %}2{% endif %}" data-store="product-item-price-{{ product.id }}">
                            {% if is_subscription_only %}
                                ...
                                {{ component('subscriptions/subscription-price', {
                                    ...
                                    subscription_classes: {
                                        prices_container: 'd-flex flex-column',
                                        price_compare: 'price-compare',
                                        price_with_subscription: 'item-price',
                                        discount_container: 'mt-1 font-small',
                                    },
                                }) }}
                            {% else %}
