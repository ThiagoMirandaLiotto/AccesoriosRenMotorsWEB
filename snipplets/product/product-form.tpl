<div class="js-sticky-product">
    <div class="product-detail-container{% if settings.positive_color_background %} container-invert{% endif %}">
        ...
        <div class="js-price-container mt-2 mb-4" data-store="product-price-{{ product.id }}">
            <span class="price-container">
                ...
                <span class="d-inline-flex align-items-center">
                   <span id="compare_price_display" class="js-compare-price-display price-compare {% if product_can_show_installments or (product.promotional_offer and not product.promotional_offer.script.is_percentage_off) %}mb-2{% endif %}" {% if not product.compare_at_price or not product.display_price %}style="display:none;"{% else %} style="display:inline;"{% endif %}>{% if product.compare_at_price and product.display_price %}{{ product.compare_at_price | money }}{% endif %}</span>
                    {{ component('promotional-price-details', {
                        svg_sprites: false,
                        promotional_price_details_custom_icon: include("snipplets/svg/tag.tpl", { svg_custom_class: "icon-inline icon-lg icon-w-14" }),
        ...
        {{ component('subscriptions/subscription-price', {
            ...
            subscription_classes: {
                container: 'price-container mt-2 mb-4',
                prices_container: 'd-flex align-items-center',
                price_compare: 'price-compare mr-2',
                price_with_subscription: 'order-first',
                discount_container: 'font-medium text-accent mt-1',
        ...
        {{ component('gift-promotion-message', {
            container_classes: {
                container: 'mb-3',
                highlight: 'text-accent font-weight-bold',
            },
        }) }}
        ...
        <form id="product_form" class="js-product-form js-fixed-product-form" method="post" action="{{ store.cart_url }}" data-store="product-form-{{ product.id }}">
            ...
            <div class="{% if product.variations %}js-product-variants{% else %}js-product-without-variants{% endif %} form-row">
                ...
                {{ component('subscriptions/subscription-selector', {
                    ...
                    subscription_classes: {
                        container: 'radio-button-container col-12 my-2',

                        radio_button: settings.positive_color_background ? 'radio-button-invert' : 'radio-button-positive',
                        radio_button_content: 'box mb-3',
                        ...
                        purchase_option_price: 'col-auto pl-2 text-right h5',
                        ...
                        dropdown_option_info: 'col pr-2',
                        dropdown_option_price: 'col-auto font-weight-bold',
                        dropdown_option_discount: 'text-accent mt-1 font-weight-bold',

                        cart_alert: 'text-center',
                        shipping_message: 'mt-3 container-invert',
                        shipping_message_title: 'h4 mb-2',
                        shipping_message_text: 'mb-5',

                        legal_message: 'font-smallest text-center mb-2',
                    ...
                    dropdown_icon: true,
                    dropdown_custom_icon: include("snipplets/svg/chevron-down.tpl", { svg_custom_class: "svg-icon-text" }),

                    legal_modal_close_custom_icon: include("snipplets/svg/times.tpl", { svg_custom_class: "svg-inline--fa svg-icon-text" }),
                }) }}
            ...
            <div class="mb-4">
                ...
                <input type="submit" class="js-addtocart js-prod-submit-form btn btn-primary btn-block mb-3 {{ state }}" value="{{ btn_value }}" {% if state == 'nostock' %}disabled{% endif %} data-store="product-buy-button"/>
                ...
                {% if not product.is_non_shippable and has_product_available and has_free_shipping and not has_product_free_shipping %}
                    ...
                    <div class="js-product-form-free-shipping-message {% if free_shipping_minimum_label_changes_visibility %}js-free-shipping-message{% endif %} text-accent font-weight-bold mb-3 w-100" {% if not cart.free_shipping.cart_has_free_shipping %}style="display: none;"{% endif %}>
